...
@section('content')
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">جميع الفواتير الارشيف</h4>
                        <a href="{{ route('invoices.create') }}" class='btn btn-outline-primary btn-inline'>اضافة
                            فاتورة</a>
                    </div>
                    <p class="tx-12 tx-gray-500 mb-2">قائمة بجميع الفواتير الارشيف لدينا..</p>
                </div>
                <div class="card-body">
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    <a class="dropdown-item modal-effect forcedel" href="#forcedelete"
                                                        data-effect="effect-scale" data-toggle="modal"
                                                        data-id="{{ $invoice->id }}">حذف
                                                        الفاتورة</a>
                                                    <a class="dropdown-item modal-effect del" href="#delete"
                                                        data-effect="effect-scale" data-toggle="modal"
                                                        data-id="{{ $invoice->id }}">ازالة من الارشيف
                                                    </a>
                                                </div>
                                            </div>
    ...
    <div class="modal" id="delete">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="{{ route('archive.destroy', 'Error') }}" method="POST" class="modal-content modal-content-demo">
                @csrf
                @method('DELETE')
                <div class="modal-header">
                    <h6 class="modal-title">ازال من الارشيف</h6><button aria-label="Close" class="close"
                        data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="id" class="hidden_input">
                    <input type="hidden" name="page" value="1">
                    <h6>هل تريد ازال الفاتورة من الارشيف؟</h6>
                </div>
                <div class="modal-footer">
                    <button class="btn ripple btn-secondary" data-dismiss="modal" type="button">الغاء</button>
                    <button class="btn ripple btn-success" type="submit">ازاله</button>
                </div>
            </form>
        </div>
    </div>
    <!-- End Modal effects-->
    <!-- Modal effects -->
    <div class="modal" id="forcedelete">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="{{ route('archive.destroy', 'Error') }}" method="POST"
                ...
                <div class="modal-body">
                    <input type="hidden" name="id" class="force_hidden_input">
                    <input type="hidden" name="page" value="2">
                    <h6>هل انت متاكد من حذف هذا الفاتورة؟</h6>
                </div>
                <div class="modal-footer">
                    <button class="btn ripple btn-secondary" data-dismiss="modal" type="button">الغاء</button>
...
@section('js')
    <script>
        ...
        // Add ID For Delete
        let links_del_forcedel = document.getElementsByClassName("forcedel");
        for (let i = 0; i < links_del_forcedel.length; i++) {
            links_del_forcedel[i].addEventListener('click', () => {
                document.getElementsByClassName("force_hidden_input")[0].value = links_del_forcedel[i].getAttribute(
                    "data-id")
            })
        }
    </script>
