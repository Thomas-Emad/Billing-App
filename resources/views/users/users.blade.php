...
@section('content')
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">عرض جميع المستخدمين</h4>
                        <div>
                            <a href="{{ route('users.create') }}" class='btn btn-outline-primary btn-inline'>اضافة
                                مستخدم</a>
                        </div>
                    </div>
                    <p class="tx-12 tx-gray-500 mb-2">قائمة بجميع المستخدمين لدينا..</p>
                </div>
                <div class="card-body">
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    ...
                                                    <a class="dropdown-item modal-effect del" href="#delete"
                                                        ...
                                                        data-id="{{ $user->id }}" data-name="{{ $user->name }}">حذف
                                                    </a>
                                                </div>
                                            </div>
                                        </td>
    ...
    <div class="modal" id="delete">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="{{ route('users.destroy', 'Error') }}" method="POST" class="modal-content modal-content-demo">
                @csrf
                @method('DELETE')
                <div class="modal-header">
                    <h6 class="modal-title">حذف هذا المستخدم</h6><button aria-label="Close" class="close"
                        data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <h6>هل تريد حذف هذا المستخدم نهائيا؟!!</h6>
                    <input type="hidden" name="id" class="hidden_input">
                    <input type="text" name="name" class="form-control name_del" disabled>
                </div>
    ...
    <script>
        ...
        for (let i = 0; i < links_del.length; i++) {
            links_del[i].addEventListener('click', () => {
                ...
                document.getElementsByClassName("name_del")[0].value = links_del[i].getAttribute("data-name")
            })
        }
    </script>
